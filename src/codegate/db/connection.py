...
class DbRecorder(DbCodeGate):
    ...
    async def record_request(self, prompt_params: Optional[Prompt] = None) -> Optional[Prompt]:
        ...
        print("in recorded request")
        print(recorded_request)
        return recorded_request  # type: ignore

    async def update_request(self, initial_id: str, prompt_params: Optional[Prompt] = None) -> Optional[Prompt]:
        print("i update request")
        if prompt_params is None:
            return None
        ...
        # Uncomment to debug the recorded request
        # logger.debug(f"Recorded request: {recorded_request}")
        print("in updated request")
        print(updated_request)
        return updated_request  # type: ignore

    async def record_outputs(self, outputs: List[Output], initial_id: Optional[str]) -> Optional[Output]:
        if not outputs:
            return
