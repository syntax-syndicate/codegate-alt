...
class ProxyUtils:
        ...
        for route in self.ghroute.VALIDATED_ROUTES:
            if path.startswith(route.path):
                ...
                logger.debug(
                    ...
                    f"(using route {route.path} -> {route.target})"
                )
                return target
    ...
    def prepare_headers(self, request: Union[Request, WebSocket], target_url: str) -> Dict[str, str]:
        ...
        for header, value in request_headers.items():
            if header.lower() in [h.lower() for h in self.ghroute.PRESERVED_HEADERS]:
                headers[header] = value

        # Add endpoint-specific headers
        if isinstance(request, Request):
            path = urlparse(str(request.url)).path
            if path in self.ghroute.ENDPOINT_HEADERS:
                headers.update(self.ghroute.ENDPOINT_HEADERS[path])
        ...
        for header in self.ghroute.REMOVED_HEADERS:
